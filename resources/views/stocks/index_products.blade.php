           ...
           <div class="text-end mb-3">
                        ...
                        @foreach($products as $product)
                            <tr> 
                                ...
                                <td class="d-flex gap-2">  
                                    @if($product->active)  
                                        <a  onclick="abrirmodaleditproduct('{{ $product->id }}')"  class="btn btn-warning btn-sm">Editar</a> 
                                        <a href="{{ route('delete-product', ['id' => $product->id]) }}" class="btn btn-danger btn-sm" onclick="return confirm('Deseja realmente desativar ?')">Desativar</a>
                                    @else 
                                        <a href="{{ route('activate-product', ['id' => $product->id]) }}" class="btn btn-success btn-sm">Reativar</a> 
                                    @endif
                                </td>  
                            </tr>
                        @endforeach
            ...
            <div class="container mt-4">
                <div class="d-flex justify-content-between align-items-center">
                    <p class="mb-0">Mostrando {{ $products->firstItem() }} a {{ $products->lastItem() }} de {{ $products->total() }} resultados</p>
                    <div>
...
<script>
    ...
    function abrirmodaleditproduct($id) { 
        // Fazer a requisição AJAX para pegar dados do produto
        $.ajax({
            url: "{{ route('product-edit', ':id') }}".replace(':id', $id),
            type: 'GET',
            success: function(data) {
                $("#exampleModalLabel").text('Editar Produto');
                $(".modal-body").html(data)
                $('#exampleModal').modal('show');
            }
        });
